...
LLM-controllable argument, and get_order_by_id can't be used to look up
another user's order by guessing an order_id.

Renamed from the original get_cart_items/get_order_status names to
get_user_cart/get_order_by_id (see tools.py) — updated here to match,
along with new coverage for get_latest_order and create_support_ticket.
"""
import sys
...
import pytest
from app.graph.tools import get_user_cart, get_order_history, get_latest_order, get_order_by_id
...
def test_user_id_is_not_visible_in_any_tool_schema():
    ...
    assert "user_id" not in get_user_cart.args
    assert "user_id" not in get_order_history.args
    assert "user_id" not in get_latest_order.args
    assert "user_id" not in get_order_by_id.args


def test_get_user_cart_uses_injected_user_id_not_an_argument():
    result = get_user_cart.invoke({}, config=_config("user_001"))
...
def test_get_latest_order_returns_the_most_recent_order():
    result = get_latest_order.invoke({}, config=_config("user_001"))
    orders = mock_db.get_order_history("user_001")
    if orders:
        assert orders[0]["order_id"] in result
    else:
        assert "no orders yet" in result


def test_get_latest_order_uses_injected_user_id_not_an_argument():
    # user_001 has orders, a never-seeded user id does not — this fails
    # if the tool were somehow reading a different/global user by mistake
    result = get_latest_order.invoke({}, config=_config("totally_unseeded_user"))
    assert "no orders yet" in result


def test_get_order_by_id_returns_data_for_the_owning_user():
    orders = mock_db.get_order_history("user_001")
    order_id = orders[0]["order_id"]
    result = get_order_by_id.invoke({"order_id": order_id}, config=_config("user_001"))
    assert order_id in result
    assert "No order found" not in result


def test_get_order_by_id_blocks_cross_user_access():
    orders = mock_db.get_order_history("user_001")
    ...
    result = get_order_by_id.invoke({"order_id": user_001_order_id}, config=_config("user_002"))
    assert "No order found" in result
    assert user_001_product not in result


def test_get_order_by_id_gives_identical_response_for_nonexistent_and_someone_elses_order():
    ...
    orders = mock_db.get_order_history("user_001")
    real_but_not_mine = get_order_by_id.invoke(
        {"order_id": orders[0]["order_id"]}, config=_config("user_002")
    )
    doesnt_exist = get_order_by_id.invoke(
