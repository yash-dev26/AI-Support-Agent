...
state. Requires the server running (uvicorn main:app) and MONGODB_URI set.

Run: python test_concurrent_escalation.py
"""
...
def run_user(user_id: str, should_escalate: bool):
    ...
    resp = requests.post(f"{BASE_URL}/chat/{user_id}", json={"message": message})
...
def main():
    ...
    pending = requests.get(f"{BASE_URL}/support/pending").json()["pending"]
    ...
    def resolve(thread_id):
        resp = requests.post(
            ...
            json={"resolution": "Refund issued, duplicate charge reversed."},
        )
    ...
    remaining = requests.get(f"{BASE_URL}/support/pending").json()["pending"]
